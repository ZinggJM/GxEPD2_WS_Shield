...
#endif
{
  public:
...
#endif
    {
      ...
      setFullWindow();
      // SRAM support
      _black_buffer =  epd2._sram_alloc(uint32_t(GxEPD2_Type::WIDTH / 8) * uint32_t(page_height));
      _color_buffer =  epd2._sram_alloc(uint32_t(GxEPD2_Type::WIDTH / 8) * uint32_t(page_height));
    }
    ...
    void drawPixel(int16_t x, int16_t y, uint16_t color)
    {
      ...
      uint8_t bb = epd2._readByte(_black_buffer + i);
      uint8_t cb = epd2._readByte(_color_buffer + i);
      bb = (bb | (1 << (7 - x % 8))); // white
      cb = (cb | (1 << (7 - x % 8)));
      if (color == GxEPD_WHITE);
      else if (color == GxEPD_BLACK) bb = (bb & (0xFF ^ (1 << (7 - x % 8))));
      else if (color == GxEPD_RED) cb = (cb & (0xFF ^ (1 << (7 - x % 8))));
      epd2._writeByte(_black_buffer + i, bb);
      epd2._writeByte(_color_buffer + i, cb);
    }
    ...
    void init(uint32_t serial_diag_bitrate = 0) // = 0 : disabled
    {
      ...
      _current_page = 0;
      setFullWindow();
      //Serial.print("_black_buffer="); Serial.print(uint32_t(_black_buffer)); Serial.print(", _color_buffer="); Serial.println(uint32_t(_color_buffer));
    }
    ...
    void fillScreen(uint16_t color) // 0x0 black, >0x0 white, to buffer
    {
      ...
      {
        epd2._writeByte(_black_buffer + x, black);
        epd2._writeByte(_color_buffer + x, red);
      }
    }

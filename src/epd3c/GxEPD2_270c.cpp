...
void GxEPD2_270c::writeImage(uint32_t black, uint32_t color, int16_t x, int16_t y, int16_t w, int16_t h, bool invert, bool mirror_y)
{
  if (_initial_write) writeScreenBuffer(); // initial full screen buffer clean
  delay(1); // yield() to avoid WDT on ESP8266 and ESP32
  int16_t wb = (w + 7) / 8; // width bytes, bitmaps are padded
...
void GxEPD2_270c::writeImagePart(uint32_t black, uint32_t color, int16_t x_part, int16_t y_part, int16_t w_bitmap, int16_t h_bitmap,

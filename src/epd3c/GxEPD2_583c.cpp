...
void GxEPD2_583c::writeImage(uint32_t black, uint32_t color, int16_t x, int16_t y, int16_t w, int16_t h, bool invert, bool mirror_y)
{
  if (_initial_write) writeScreenBuffer(); // initial full screen buffer clean
  delay(1); // yield() to avoid WDT on ESP8266 and ESP32
  uint16_t wb = (w + 7) / 8; // width bytes, bitmaps are padded
  ...
  for (int16_t i = 0; i < h1; i++)
  {
    for (int16_t j = 0; j < w1 / 8; j++)
    {
      ...
      if (black)
      {
        // use wb, h of bitmap for index!
        uint16_t idx = mirror_y ? j + dx / 8 + uint16_t((h - 1 - (i + dy))) * wb : j + dx / 8 + uint16_t(i + dy) * wb;
        black_data = _readByte(black + idx);
        if (invert) black_data = ~black_data;
      }
      if (color)
      {
        // use wb, h of bitmap for index!
        uint16_t idx = mirror_y ? j + dx / 8 + uint16_t((h - 1 - (i + dy))) * wb : j + dx / 8 + uint16_t(i + dy) * wb;
        color_data = _readByte(color + idx);
        if (invert) color_data = ~color_data;
      }
      _send8pixel(~black_data, ~color_data);
    }
...
void GxEPD2_583c::writeImagePart(uint32_t black, uint32_t color, int16_t x_part, int16_t y_part, int16_t w_bitmap, int16_t h_bitmap,
